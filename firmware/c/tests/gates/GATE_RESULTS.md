...
- **Soak — DONE (pre-soak `probe-rs info` on GP26/27 read both cores' ROM tables):**
  ...
  - ⚠️ **CAVEAT — these counts measure the PROBE, and assume a HEALTHY, well-powered TARGET.** A clean run needs the target's AHB-AP up (a HardFault-locked or QSPI-brown-out target cannot run the flash ROM functions and will verify-mismatch no matter how good the probe is). Two confounds seen repeatedly: (1) a back-to-back *sustained* soak brown-out-glitches this particular target Pico W — power-cycle the TARGET between long runs (M4_RESULTS.md §R1); (2) a locked target (the only failure mode `recover.sh` hit on 2026-06-21). The harness now classifies these as `TARGET-GLITCH` (AHB-AP gone) and reports `target_glitch=` separately from the PROBE verdict — see `lib_target_state.sh`. Re-confirmed clean 2026-06-21 on a healthy independently-powered target: probe-rs **40/40** + openocd **20/20**, 0 fails / 0 stalls.
